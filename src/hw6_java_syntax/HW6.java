...
public class HW6 {
	...
	public static void main(String[] args) {
		...
		System.out.println(Integer.toBinaryString(bin1 & bin2)); // Binary AND Operator copies a bit to the result if it
																	// exists in both operands.
		System.out.println(Integer.toBinaryString(bin1 | bin2)); // Binary OR Operator copies a bit if it exists in
																	// either operand.
		System.out.println(Integer.toBinaryString(bin1 ^ bin2)); // Binary XOR Operator copies the bit if it is set in
																	// one operand but not both.
		System.out.println(Integer.toBinaryString(~bin1)); // Binary Ones Complement Operator is unary and has the
															// effect of 'flipping' bits.
		System.out.println("Bindary Right shift: " + (bin2 >> 2));// Binary Right Shift Operator. The left operands
																	// value is moved right by the number of bits
																	// specified by the right operand.
		System.out.println("Shift right zero fill: " + (bin2 >>> 2));// Shift right zero fill operator. The left
